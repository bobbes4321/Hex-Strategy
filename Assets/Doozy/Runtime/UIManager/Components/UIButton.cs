...
namespace Doozy.Runtime.UIManager.Components
{
    ...
    {
        #if UNITY_EDITOR
        [UnityEditor.MenuItem("GameObject/Doozy/UI/Components/UIButton", false, 8)]
        ...
        public static UIButton GetButton(string category, string name) =>
            database.Where(button => button.Id.Category.Equals(category)).First(button => button.Id.Name.Equals(name));
